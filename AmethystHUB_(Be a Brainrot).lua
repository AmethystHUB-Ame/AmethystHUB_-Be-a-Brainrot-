--[[
  Amethyst Brainrot V3 - Full Optimized Script
...
local function loadConfigFromClipboard()
    safeCall(function()
        ...
        if data and data.S then
            ...
            -- Refresh UI (will be done after UI rebuild)
        else
...
local function createScreenGui(name, displayOrder)
    ...
    gui:SetAttribute("__AmethystV3", true)
...
-- Component factories (simplified but complete)
...
local function checkForStaff()
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer then
            ...
            -- Optional: check group roles
            -- local rank = player:GetRoleInGroup(0)
            -- for _, sr in ipairs(staffRanks) do if rank and string.find(rank, sr) then return true end end
        end
    end
    return false
end
...
-- HOME TAB (for live stats) - create a new page and add it
...
-- recreate tab buttons for Home (simple, just add)
...
Amethyst.cleanup = function()
    ...
    for _, gui in ipairs(CoreGui:GetChildren()) do if gui:GetAttribute("__AmethystV3") then gui:Destroy() end end
    local pg = LocalPlayer:FindFirstChildOfClass("PlayerGui")
    if pg then for _, gui in ipairs(pg:GetChildren()) do if gui:GetAttribute("__AmethystV3") then gui:Destroy() end end end
