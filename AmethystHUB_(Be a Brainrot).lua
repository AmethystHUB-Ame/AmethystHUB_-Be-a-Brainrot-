--[[
  Amethyst Brainrot V3 - Full Optimized Script (Corrected & Optimized)
...
--[[ DOUBLE EXECUTION GUARD & INITIALIZATION ]]--------------------------------
...
-- Initialize connections table at the very top
local connections = {}
...
--[[ STATE TABLE (Properly Defined with All Defaults) ]]----------------------
local S = {
    -- Farm tab
    AutoCollect = false,
    CollectInterval = 1,
...
setmetatable(S, { __newindex = function(t, k, v) rawset(t, k, v); for _, cat in pairs(_S_categories) do if rawget(cat, k) ~= nil then cat[k] = v return end end end })
...
-- Define all TweenInfos including TI_TAB
...
local TI_TAB = TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
...
local function createScreenGui(name, displayOrder)
    ...
    gui:SetAttribute("__AmethystBrainrotV3", true) -- Consistent attribute name
...
-- Fixed createParagraph to return Content label reference
local function createParagraph(parent, config)
    ...
    contentLbl.TextWrapped = true
    contentLbl.Parent = frame
    -- Return both frame and content label for updating
    return {Frame = frame, Content = contentLbl}
end
...
local function collectLoop()
    while Amethyst._alive and collectActive do
        for slot = 1, 20 do
            if not Amethyst._alive or not collectActive then break end
...
local function claimLoop()
    while Amethyst._alive and claimActive do
        for i = 1, 9 do
            if not Amethyst._alive or not claimActive then break end
    ...
    while Amethyst._alive and sellActive do
        ...
        if fillPercent >= S.SellThreshold then
            ...
            for slotKey, brainrot in pairs(stored) do
                if not Amethyst._alive or not sellActive then break end
                ...
                if data then
                    ...
                    if S.ExcludedRarities[rarity] then excluded = true end
                    if not excluded and S.ExcludedMutations[mutation] then excluded = true end
                    if not excluded and S.ExcludedNames[index] then excluded = true end
...
-- Smart Auto Upgrade Brainrots (with slot cache) - Fixed: Removed 'continue'
...
local function upgradeBrainrotLoop()
    while Amethyst._alive and upgradeBrainrotActive do
        safeCall(function()
            ...
            for slot = 1, 30 do
                if not upgradeBrainrotActive or not Amethyst._alive then break end
                
                -- Fixed: Removed 'continue' statement, using if-else instead
                if not (upgradedSlotsCache[plotId] and upgradedSlotsCache[plotId][slot]) then
...
-- ============================================================
-- TAB UI BUILDING (with new elements)
-- ============================================================
...
createButton(tabPages.Automation, {Name = "Sell All (Filters Apply)", Callback = function() safeCall(sellLoop) end})
...
-- Stats update loop - Fixed: Properly reference Content label
task.spawn(function()
    while Amethyst._alive do
        updateStats()
        local fillPercent = getInventoryFillPercentage()
...
-- Cleanup handler - Fixed: Consistent attribute naming
Amethyst.cleanup = function()
    ...
    -- Use correct attribute name for GUI cleanup
