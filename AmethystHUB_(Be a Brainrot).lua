--[[
  ...
  - Smart Auto-Upgrade with slot caching
  - Sell threshold & inventory percentage
  - Dynamic remote rate limiting (anti-lag / anti-disconnect)
  - Live stats window (Rebirths, Money/min)
  - Config profiles (copy/load JSON)
  - Anti-staff detection (auto-disable on admin/mod join)
  - Randomized human-like delays
  - Fully merged with Amethyst UI Framework
]]
...
-- Remote call with safety (fixed FireServer)
local function safeFireRemote(remoteName, ...)
    ...
    elseif remote and type(remote.Fire) == "function" then
        -- fallback for some exploit environments
...
--[[ STATE TABLE (default values) ]]-----------------------------------------
local S = {
    -- Farm tab
    AutoCollect = false,
    CollectInterval = 1,
    FarmBrainrot = false,
    FarmRarityFilters = {},      -- list of strings
    FarmMutationFilters = {},    -- list of strings
    ...
    AutoSell = false,
    SellInterval = 3,
    SellThreshold = 90,           -- sell when inventory >= X%
    SellMode = "Exclude",
    ExcludedRarities = {},        -- list of strings
    ExcludedMutations = {},       -- list of strings
    ExcludedNames = {},           -- list of strings
...
-- Auto-save
task.spawn(function() while Amethyst._alive do task.wait(30); saveConfig() end end)

--[[ THEME CONSTANTS (Frosted Amethyst) ]]-----------------------------------
local THEME = {
    Background = Color3.fromRGB(13,11,20), Surface = Color3.fromRGB(22,18,31),
    SurfaceElevated = Color3.fromRGB(30,26,42), Border = Color3.fromRGB(42,36,64),
    Accent = Color3.fromRGB(155,89,240), AccentGlow = Color3.fromRGB(180,122,255),
    AccentSecondary = Color3.fromRGB(108,61,207), TextPrimary = Color3.fromRGB(238,234,245),
    TextSecondary = Color3.fromRGB(139,131,158), Success = Color3.fromRGB(74,222,128),
    Danger = Color3.fromRGB(248,113,113), Warning = Color3.fromRGB(251,191,36),
    White = Color3.fromRGB(255,255,255), ElementBackground = Color3.fromRGB(22,18,31),
    ElementBackgroundHover = Color3.fromRGB(30,26,42), ElementStroke = Color3.fromRGB(42,36,64),
    SliderBackground = Color3.fromRGB(22,18,31), SliderProgress = Color3.fromRGB(155,89,240),
    ToggleEnabled = Color3.fromRGB(155,89,240), ToggleDisabled = Color3.fromRGB(30,26,42),
    InputBackground = Color3.fromRGB(18,15,26), InputStroke = Color3.fromRGB(42,36,64),
    PlaceholderColor = Color3.fromRGB(100,90,130), WindowBackground = Color3.fromRGB(13,11,20),
    Topbar = Color3.fromRGB(18,15,26), SidebarBackground = Color3.fromRGB(16,13,24),
    SidebarHover = Color3.fromRGB(25,22,36), ContentBackground = Color3.fromRGB(13,11,20),
    LoadingBackground = Color3.fromRGB(8,6,14), LoadingBarBackground = Color3.fromRGB(30,26,42),
...
local function tweenProp(obj, props, tweenInfo) safeCall(function() TweenService:Create(obj, tweenInfo or TI_SMOOTH, props):Play() end) end
...
local function processQueue()
    local visible = 0
    for _, child in ipairs(notifContainer:GetChildren()) do if child:IsA("Frame") and child.Name == "Notification" then visible = visible + 1 end end
    while visible < MAX_VISIBLE and #notifQueue > 0 do table.remove(notifQueue, 1).show(); visible = visible + 1 end
end
local function notify(title, content, dur, severity)
    dur = dur or 4
    local severityColor = severity == "success" and THEME.Success or severity == "danger" and THEME.Danger or severity == "warning" and THEME.Warning or THEME.Accent
...
local function notifyToggle(name, state) notify(name .. (state and " ON" or " OFF"), state and "Enabled" or "Disabled", 3) end
...
-- Loading Screen (will be shown first, then replaced)
...
-- Main Hub Window (will be created after loading, but we need to define it for the animation)
local hubGui, mainFrame, sidebar, contentArea, tabPages, tabButtons, watermarkGui, toggleGui, toggleBtn, toggleGlow

--[[ COMPONENT FACTORIES (must be defined before UI building) ]]---------------
...
hubGui = createScreenGui("AmethystHub", 10)
...
-- Tabs
local TAB_NAMES = {"Home", "Farm", "Upgrades", "Automation", "Misc", "Settings"}
tabPages = {}
...
local function switchTab(tabName)
    ...
    for name, btnData in pairs(tabButtons) do
        ...
        tweenProp(btnData.button, {BackgroundColor3 = isSelected and THEME.SurfaceElevated or THEME.SidebarBackground}, TI_SMOOTH)
...
-- Watermark
watermarkGui = createScreenGui("AmethystWatermark", 999)
...
toggleGui = createScreenGui("AmethystToggle", 1000)
...
--[[ LOADING ANIMATION (after mainFrame exists) ]]---------------------------
task.spawn(function()
...
-- ============================================================
-- BRAINROT BACKEND LOGIC (with improvements)
-- ============================================================
...
-- Original functions (with safe calls)
...
local function SpeedUpgrade(amount)
    safeFireRemote("SpeedUpgrade", amount)
end
...
local function SellBrainrots()
    ...
    local excludedRaritiesSet = listToSet(S.ExcludedRarities)
    local excludedMutationsSet = listToSet(S.ExcludedMutations)
    local excludedNamesSet = listToSet(S.ExcludedNames)
    for slotKey, brainrot in pairs(stored) do
        ...
        if data then
            ...
            if excludedRaritiesSet[rarity] then excluded = true end
            if not excluded and excludedMutationsSet[mutation] then excluded = true end
            if not excluded and excludedNamesSet[index] then excluded = true end
            local shouldSell = (S.SellMode == "Exclude" and not excluded) or (S.SellMode == "Include" and excluded)
            if shouldSell then
...
local function getInventoryFillPercentage()
    local stored = GetData("StoredBrainrots") or {}
    local total = 0
    for _ in pairs(stored) do total = total + 1 end
    local maxSlots = 30
    pcall(function()
...
-- Auto loops with adaptive waits
...
-- Farm Brainrots (teleport and steal) with improved prompt detection
local function modelMatchesFilters(model)
    ...
    if slotRef then
        ...
        if slotNum then
            ...
            if slotNum >= 9 and not hasVIP then return false end
        end
    end
    local selRaritiesSet = listToSet(S.FarmRarityFilters)
    local selMutationsSet = listToSet(S.FarmMutationFilters)
    if next(selRaritiesSet) == nil and next(selMutationsSet) == nil then return true end
    local rarity = model:GetAttribute("Rarity")
    local mutation = model:GetAttribute("Mutation")
    if rarity and selRaritiesSet[rarity] then return true end
    if mutation then
        if selMutationsSet[mutation] then return true end
    else
        if selMutationsSet["Normal"] then return true end
    end
    return false
end
...
-- Smart Auto Upgrade Brainrots (with slot cache, fixed continue)
...
local staffRanks = {"Admin", "Moderator", "Owner", "Developer", "Staff"}
...
createSection(farmPage, "Collection")
local collectToggle = createToggle(farmPage, {Name = "Auto Collect Cash", CurrentValue = S.AutoCollect, Callback = function(v)
    S.AutoCollect = v
    collectActive = v
...
local collectSlider = createSlider(farmPage, {Name = "Collect Interval", Range = {0.5, 10}, Increment = 0.5, CurrentValue = S.CollectInterval, Callback = function(v) S.CollectInterval = v end})

createSection(farmPage, "Brainrot Farming")
local farmToggle = createToggle(farmPage, {Name = "Farm Selected Brainrots", CurrentValue = S.FarmBrainrot, Callback = function(v)
    S.FarmBrainrot = v
    farmActive = v
...
local rarityDropdown = createDropdown(farmPage, {Name = "Rarity Filter", Options = rarityOptions, MultipleOptions = true, CurrentOption = S.FarmRarityFilters, Callback = function(v) S.FarmRarityFilters = v end})
local mutationOptions = {"Normal","Gold","Diamond","Rainbow","Candy"}
local mutationDropdown = createDropdown(farmPage, {Name = "Mutation Filter", Options = mutationOptions, MultipleOptions = true, CurrentOption = S.FarmMutationFilters, Callback = function(v) S.FarmMutationFilters = v end})

-- UPGRADES TAB
local upgradesPage = tabPages["Upgrades"]
createSection(upgradesPage, "Rebirth")
local rebirthToggle = createToggle(upgradesPage, {Name = "Auto Rebirth", CurrentValue = S.AutoRebirth, Callback = function(v)
    S.AutoRebirth = v
    rebirthActive = v
...
local rebirthSlider = createSlider(upgradesPage, {Name = "Rebirth Interval", Range = {0.5, 10}, Increment = 0.5, CurrentValue = S.RebirthInterval, Callback = function(v) S.RebirthInterval = v end})
createButton(upgradesPage, {Name = "Rebirth Once", Callback = function() safeFireRemote("RequestRebirth") end})

createSection(upgradesPage, "Speed Upgrades")
local speedToggle = createToggle(upgradesPage, {Name = "Auto Upgrade Speed", CurrentValue = S.AutoSpeedUpgrade, Callback = function(v)
...
local speedInterval = createSlider(upgradesPage, {Name = "Upgrade Speed Interval", Range = {0.5, 10}, Increment = 0.5, CurrentValue = S.SpeedUpgradeInterval, Callback = function(v) S.SpeedUpgradeInterval = v end})
local speedAmount = createDropdown(upgradesPage, {Name = "Upgrade Speed Amount", Options = {"1","5","10"}, CurrentOption = tostring(S.SpeedUpgradeAmount), Callback = function(v) S.SpeedUpgradeAmount = tonumber(v) end})

createSection(upgradesPage, "Base Upgrades")
local baseToggle = createToggle(upgradesPage, {Name = "Auto Upgrade Base", CurrentValue = S.AutoUpgradeBase, Callback = function(v)
    S.AutoUpgradeBase = v
    baseActive = v
...
local baseInterval = createSlider(upgradesPage, {Name = "Upgrade Base Interval", Range = {0.5, 10}, Increment = 0.5, CurrentValue = S.UpgradeBaseInterval, Callback = function(v) S.UpgradeBaseInterval = v end})
createButton(upgradesPage, {Name = "Upgrade Base Once", Callback = function() safeFireRemote("UpgradeBase") end})

createSection(upgradesPage, "Brainrot Upgrades")
local upgradeToggle = createToggle(upgradesPage, {Name = "Auto Upgrade Brainrots", CurrentValue = S.AutoUpgradeBrainrots, Callback = function(v)
...
local maxLevelInput = createInput(upgradesPage, {Name = "Max Upgrade Level", PlaceholderText = "Enter max level...", Callback = function(v) local num = tonumber(v); if num then S.MaxUpgradeLevel = num end end, RemoveTextAfterFocusLost = false})

-- AUTOMATION TAB
local automationPage = tabPages["Automation"]
createSection(automationPage, "Best Brainrots")
local equipToggle = createToggle(automationPage, {Name = "Auto Equip Best Brainrots", CurrentValue = S.AutoEquipBest, Callback = function(v)
    S.AutoEquipBest = v
    equipActive = v
...
local equipInterval = createSlider(automationPage, {Name = "Equip Best Interval", Range = {0.5, 10}, Increment = 0.5, CurrentValue = S.EquipBestInterval, Callback = function(v) S.EquipBestInterval = v end})

createSection(automationPage, "Gifts / Rewards")
local claimToggle = createToggle(automationPage, {Name = "Auto Claim Free Gifts", CurrentValue = S.AutoClaimGifts, Callback = function(v)
    S.AutoClaimGifts = v
    claimActive = v
...
local claimInterval = createSlider(automationPage, {Name = "Claim Gifts Interval", Range = {0.5, 10}, Increment = 0.5, CurrentValue = S.ClaimGiftsInterval, Callback = function(v) S.ClaimGiftsInterval = v end})

createSection(automationPage, "Auto Sell")
local sellToggle = createToggle(automationPage, {Name = "Enable Auto Sell", CurrentValue = S.AutoSell, Callback = function(v)
    S.AutoSell = v
    sellActive = v
...
local sellIntervalSlider = createSlider(automationPage, {Name = "Sell Interval", Range = {0.5, 10}, Increment = 0.5, CurrentValue = S.SellInterval, Callback = function(v) S.SellInterval = v end})
local sellThresholdSlider = createSlider(automationPage, {Name = "Sell Threshold (%)", Range = {0, 100}, Increment = 5, CurrentValue = S.SellThreshold, Callback = function(v) S.SellThreshold = v end})
local modeDropdown = createDropdown(automationPage, {Name = "Sell Mode", Options = {"Exclude Selected", "Exclude Non Selected"}, CurrentOption = (S.SellMode == "Exclude" and "Exclude Selected" or "Exclude Non Selected"), Callback = function(v) S.SellMode = (v == "Exclude Selected" and "Exclude" or "Include") end})

local rarityList = {}
for rarity in pairs(RaritiesData) do table.insert(rarityList, rarity) end
table.sort(rarityList)
local rarityExclude = createDropdown(automationPage, {Name = "Filter Rarities", Options = rarityList, MultipleOptions = true, CurrentOption = S.ExcludedRarities, Callback = function(v) S.ExcludedRarities = v end})
...
local mutationExclude = createDropdown(automationPage, {Name = "Filter Mutations", Options = mutationList, MultipleOptions = true, CurrentOption = S.ExcludedMutations, Callback = function(v) S.ExcludedMutations = v end})
local nameList = {}
...
local nameExclude = createDropdown(automationPage, {Name = "Filter Names", Options = nameList, MultipleOptions = true, CurrentOption = S.ExcludedNames, Callback = function(v) S.ExcludedNames = v end})
createButton(automationPage, {Name = "Sell All (Filters Apply)", Callback = function() safeCall(SellBrainrots) end})

-- MISC TAB
local miscPage = tabPages["Misc"]
createSection(miscPage, "Useful")
local lasersToggle = createToggle(miscPage, {Name = "Remove Laser Doors", CurrentValue = S.LasersRemove, Callback = function(v)
    S.LasersRemove = v
    if v then deleteLasers() else restoreLasers() end
    notifyToggle("Remove Lasers", v)
end})
local antiShakeToggle = createToggle(miscPage, {Name = "Anti Camera Shake", CurrentValue = S.AntiShake, Callback = function(v)
    S.AntiShake = v
    antiShakeEnabled = v
    notifyToggle("Anti Shake", v)
end})
local chaseToggle = createToggle(miscPage, {Name = "Freeze Chasing Bosses", CurrentValue = S.FreezeChasingBosses, Callback = function(v)
    S.FreezeChasingBosses = v
    freezeChaseActive = v
    if v then startShakeDetection() else stopShakeDetection() end
    notifyToggle("Freeze Chasing Bosses", v)
end})
local badToggle = createToggle(miscPage, {Name = "Freeze Bad Bosses", CurrentValue = S.FreezeBadBosses, Callback = function(v)
    S.FreezeBadBosses = v
...
createButton(settingsPage, {Name = "Save Config", Callback = function() saveConfig(); notify("Config Saved", "Settings saved to file.", 3, "success") end})
createButton(settingsPage, {Name = "Load Config", Callback = function() loadConfig(); notify("Config Loaded", "Settings loaded from file.", 3, "success") end})
createButton(settingsPage, {Name = "Copy Config to Clipboard", Callback = copyConfigToClipboard})
createButton(settingsPage, {Name = "Load Config from Clipboard", Callback = loadConfigFromClipboard})
local antiStaffToggle = createToggle(settingsPage, {Name = "Anti-Staff Detection", CurrentValue = S.AntiStaff, Callback = function(v)
    S.AntiStaff = v
    if v and checkForStaff() then onStaffJoined() end
    notifyToggle("Anti-Staff", v)
end})
createButton(settingsPage, {Name = "Reset All Settings", Callback = function()
    for k, v in pairs({
...
task.spawn(function()
    while Amethyst._alive do
        updateStats()
        local fillPercent = getInventoryFillPercentage()
        local content = string.format("Rebirths this session: %d\nMoney per minute: %.0f\nInventory fill: %.1f%%", sessionRebirths, moneyPerMinute, fillPercent)
        if statsPara and statsPara.Content then
            statsPara.Content.Text = content
        end
        adaptiveWait(2)
    end
end)
...
Amethyst.cleanup = function()
    ...
    stopShakeDetection(); restoreBadBosses(); restoreLasers()
    saveConfig()
    for _, conn in ipairs(connections) do pcall(function() conn:Disconnect() end) end
    for _, gui in ipairs(CoreGui:GetChildren()) do if gui:GetAttribute("__AmethystBrainrotV3") then gui:Destroy() end end
    local pg = LocalPlayer:FindFirstChildOfClass("PlayerGui")
    if pg then for _, gui in ipairs(pg:GetChildren()) do if gui:GetAttribute("__AmethystBrainrotV3") then gui:Destroy() end end end
